...
{
    private const string RoutePrefix = "/api/ratings";

    public override void Handle(HttpRestEventArgs e)
    {

        if (Regex.Match(e.Path, @"^/api/ratings/(?<id>[^/]+)/rate$").Success && e.Method == HttpMethod.Post)
        {
            try
            {
                var match = Regex.Match(e.Path, @"^/api/ratings/(?<id>[^/]+)/rate$");
                ...
                var rating = new Rating(stars, comment, session.UserId, mediaId);
        ...
        {
            var match = Regex.Match(e.Path, @"^/api/ratings/(?<id>[^/]+)$");
            int mediaId = int.Parse(match.Groups["id"].Value);
        ...
        else if (Regex.Match(e.Path, @"^/api/ratings/(?<id>[^/]+)$").Success && e.Method == HttpMethod.Put)
        {
            try
            {
                var match = Regex.Match(e.Path, @"^/api/ratings/(?<id>[^/]+)$");
                ...
                Rating r = Rating.Get(mediaId, session);
                r.BeginEdit(session);
                if(stars != -1) r.stars = stars;
                if(comment != "") r.comment = comment;
                r.comment_confirmed = false;
                r.Save();
        ...
        }else if (Regex.Match(e.Path, @"^/api/ratings/(?<id>[^/]+)/confirm$").Success && e.Method == HttpMethod.Post)
        {
            try
            {
                var match = Regex.Match(e.Path, @"^/api/ratings/(?<id>[^/]+)/confirm$");
                int mediaId = int.Parse(match.Groups["id"].Value);
